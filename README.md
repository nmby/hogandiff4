...
> **お知らせ（2026年9月7日）**
>
> - 本リポジトリは旧バージョン（Java 版・v0.27.2 以前）のものです。現行バージョン（v1.0.0 以降）は[公式サイト](https://hogandiff.hotchpotch.xyz/)で公開しています。
> - **旧バージョンの Google ドライブ連携機能は 2026年12月31日をもってサポートを終了します。** 対象となる方・終了後の動作・お願いは[サポート終了のご案内](https://hogandiff.hotchpotch.xyz/notice/google-drive/)をご覧ください。
...
- PC上のExcelファイルに加えて、Googleドライブ上のExcelファイル、Googleスプレッドシートも比較することができます。（※ Googleドライブ連携機能は2026年12月31日でサポートを終了します）
- 特定のシート同士、ブック同士、フォルダ同士を比較することができます。
- 行や列の挿入や削除によるギャップを自動で検出できます。
- セルの内容が数式の場合、数式文字列／計算結果の値のどちらで比較を行うかを指定できます。
- .xls/.xlsx/.xlsm形式のExcelファイルのワークシートおよびGoogleスプレッドシートに対応しています。異なる形式のファイル同士を比較することもできます。（※ Googleスプレッドシートは2026年12月31日でサポートを終了します）
- 日本語・英語・簡体中国語を選択できます。
