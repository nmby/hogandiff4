...
import xyz.hotchpotch.hogandiff.logic.plain.BookInfoLoaderCombined;
import xyz.hotchpotch.hogandiff.logic.poi.BookInfoLoaderWithPoiEventApi;
import xyz.hotchpotch.hogandiff.logic.poi.BookInfoLoaderWithPoiUserApi;
import xyz.hotchpotch.hogandiff.logic.sax.BookInfoLoaderWithSax;
...
public interface BookInfoLoader {
     ...
     */
    public static BookInfoLoader of(Path bookPath) {
        Objects.requireNonNull(bookPath);
        
        Set<SheetType> targetSheetTypes = EnumSet.of(SheetType.WORKSHEET);
        
        return switch (BookType.of(bookPath)) {
        case XLS -> BookInfoLoaderCombined.of(List.of(
                () -> BookInfoLoaderWithPoiEventApi.of(targetSheetTypes),
                () -> BookInfoLoaderWithPoiUserApi.of(targetSheetTypes)));
    
        case XLSX, XLSM -> BookInfoLoaderCombined.of(List.of(
                () -> BookInfoLoaderWithSax.of(targetSheetTypes),
                () -> BookInfoLoaderWithPoiUserApi.of(targetSheetTypes)));
