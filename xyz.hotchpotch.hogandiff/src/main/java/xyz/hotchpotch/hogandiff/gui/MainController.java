...
public class MainController extends VBox {
    ...
    public void bindSheetCompareInfoProp() {
        sheetCompareInfoProp.bind(Bindings.createObjectBinding(
                () -> {
                    ...
                    BookInfoComparison prevValue = ar.settings().get(SettingKeys.CURR_SHEET_COMPARE_INFO);
                    ...
                    switch (menu) {
                        case COMPARE_SHEETS:
                            ...
                            if (prevValue != null
                                ...
                                return new BookInfoComparison(bookInfoPair, List.of(sheetNamePair));
                            }
                        case COMPARE_BOOKS:
                        case COMPARE_DIRS:
                        case COMPARE_TREES:
    ...
    public void bindBookCompareInfoProp() {
        bookCompareInfoProp.bind(Bindings.createObjectBinding(
                () -> {
                    AppMenu menu = menuProp.getValue();
                    Pair<BookInfo> bookInfoPair = bookInfoPropPair.map(Property::getValue);
                    BookInfoComparison prevValue = ar.settings().get(SettingKeys.CURR_BOOK_COMPARE_INFO);
                    ...
                    switch (menu) {
                        case COMPARE_BOOKS:
                            ...
                            if (prevValue != null && bookInfoPair.equals(prevValue.parentBookInfoPair())) {
                                ...
                                return BookInfoComparison.calculate(
                                        bookInfoPair,
    ...
    public void bindDirCompareInfoProp() {
        dirCompareInfoProp.bind(Bindings.createObjectBinding(
                () -> {
                    ...
                    DirInfoComparison prevValue = ar.settings().get(SettingKeys.CURR_DIR_COMPARE_INFO);
                    ...
                    switch (menu) {
                        case COMPARE_DIRS:
                            ...
                            if (prevValue != null && dirInfoPair.equals(prevValue.parentDirInfoPair())) {
                                ...
                                return DirInfoComparison.calculate(
                                        dirInfoPair,
    ...
    public void bindTreeCompareInfoProp() {
        treeCompareInfoProp.bind(Bindings.createObjectBinding(
                () -> {
                    AppMenu menu = menuProp.getValue();
                    Pair<DirInfo> dirInfoPair = dirInfoPropPair.map(Property::getValue);
                    DirInfoComparison prevValue = ar.settings().get(SettingKeys.CURR_TREE_COMPARE_INFO);
    ...
    private boolean isPasswordUsed() {
        ...
        Stream<Path> bookPathStream = switch (menu) {
            case COMPARE_SHEETS -> {
                BookInfoComparison bookInfoComparison = ar.settings().get(SettingKeys.CURR_SHEET_COMPARE_INFO);
            ...
            case COMPARE_BOOKS -> {
                BookInfoComparison bookInfoComparison = ar.settings().get(SettingKeys.CURR_BOOK_COMPARE_INFO);
                Pair<Path> bookPathPair = bookInfoComparison.parentBookInfoPair().map(BookInfo::bookPath);
                yield Stream.of(bookPathPair.a(), bookPathPair.b()).filter(bookPath -> bookPath != null);
            }
            case COMPARE_DIRS -> {
                DirInfoComparison dirInfoComparison = ar.settings().get(SettingKeys.CURR_DIR_COMPARE_INFO);
                yield bookPathStream(dirInfoComparison);
            }
            case COMPARE_TREES -> {
                FlattenDirInfoComparison flattenDirInfoComparison = ar.settings()
                        .get(SettingKeys.CURR_TREE_COMPARE_INFO)
                        .flatten();
                yield flattenDirInfoComparison.dirInfoComparisons().values().stream()
    ...
    private Stream<Path> bookPathStream(DirInfoComparison dirInfoComparison) {
